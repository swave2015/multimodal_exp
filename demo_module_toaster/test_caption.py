...
source_path = '../single_videos/toaster_caption'
    ...
    while True:
        ...
        if frame_counter % sample_rate == 0:
            if open_blip:
                ...
                answer = model_blip.generate({"image": infer_image})
                if answer[0] != '':
                    caption_show = answer[0]
                    if 'refrigerator' in caption_show:
                        caption_show = caption_show.replace('refrigerator', 'toaster')
