...
def main(args, ds_init):
    ...
    print('args_print: ', args)
    prompt_list = ['person', 'cat', 'dog', 'car', 'microwave']
    num_max_bpe_tokens = 64
    max_len = num_max_bpe_tokens
    input_image_path = '/home/caoxh/multimodal_exp/test_imgs/ovon.jpg'
    ...
    image = default_loader(input_image_path)
    input_img = transform(image)
    input_img = input_img.unsqueeze(0).cuda()
    print('input_img_shape_out: ', input_img.shape)
    ...
    print('model_config_type_print: ', type(model_config))
    print("model_config = %s" % model_config)
    print("vocab_size = %s" %  args.vocab_size)
    print("checkpoint_activations = %s" % args.checkpoint_activations)
    print('model_config_print: ', model_config)
    print('drop_path: ', args.drop_path)
    model = create_model(
        model_config,
        pretrained=False,
    ...
    print('args.finetune_print: ', args.finetune)

    if args.finetune:
        print('args_need_finetune')
    ...
    print('model_to_device----------------------------------------------------------------------------')
    print('input_img_shape: ', input_img.shape)
    print('input_text_tensor_shape: ', input_text_tensor.shape)
    print('input_padding_mask_tensor_shape: ', input_padding_mask_tensor.shape)
    vision_cls, _ = model(image=input_img, only_infer=True)
    _, language_cls = model(
            text_description=input_text_tensor, padding_mask=input_padding_mask_tensor, only_infer=True)
    print('vision_cls_shape: ', vision_cls.shape)
